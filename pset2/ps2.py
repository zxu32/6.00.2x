...
from pset2.ps2_visualize import *
import pylab
...
class RectangularRoom(object):
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        if (int(m), int(n)) in self.cleanedTiles:
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        # initial tile is cleaned at robot instantiation
        ...
        if self.room.isPositionInRoom(newPosition):
            ...
            # keep getting new position until bot is in room
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        while room.getNumCleanedTiles()/room.getNumTiles() < min_coverage:
            ...
            timeTicks += 1
        timeStepRecord.append(timeTicks)
    return sum(timeStepRecord)/len(timeStepRecord)
...
"""
    ...
    for i in range(num_trials):
        ...
        room = RectangularRoom(width, height)
        robots = [robot_type(room, speed) for i in range(num_robots)]
        environment = RobotVisualization(num_robots, width, height)

        for robot in robots:
            environment._draw_robot(robot.getRobotPosition(), robot.getRobotDirection())
        while room.getNumCleanedTiles() / room.getNumTiles() < min_coverage:
            for robot in robots:
                robot.updatePositionAndClean()
            environment.update(room, robots)

        timeStepRecord.append(environment.time)
        timeStepRecord.append(timeTicks)
    return sum(timeStepRecord) / len(timeStepRecord)
"""
